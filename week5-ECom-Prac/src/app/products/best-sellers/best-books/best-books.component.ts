...
export class BestBooksComponent implements OnInit {
  ...
  private FetchDataCarousel(){
    ...
    .subscribe(post=>{
     // console.log(post);
      ...
      this.BestSellerCarouselImg=this.CarouselImage.filter(post => post.SubCategory === 'BestSellerCarousel').splice(1,2)
  ...
  private FetchData(){
    ...
    .subscribe(post=>{
      ...
      this.isFetching=false
      this.loadedPosts = post;
      this.BooksList = this.loadedPosts.filter(post => post.ProductCategory === 'Books').splice(0,4)
    },
    )
  }
