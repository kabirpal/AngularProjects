...
export class CardsComponent implements OnInit {
  ...
  private FetchProductData(){
    ...
    .subscribe(post=>{
      ...
      this.productSmartPhone = post.filter(post=>post.ProductCategory==='SmartPhone').splice(0,4)
      this.productBooks = post.filter(post=>post.ProductCategory==='Books').splice(0,4)
      this.productMen = post.filter(post=>post.ProductCategory==='Men').splice(0,4)
      this.productWomen = post.filter(post=>post.ProductCategory==='Women').splice(0,4)
    },
    )
  }
